...
# These are whole-table aggregates against a 5 DTU Basic database, recomputed on every
# page load. The results are global rather than per-user — nothing here is filtered by
# the caller — so one cached entry serves every HR admin without leaking anything.
...
def metrics(db: Session = Depends(get_db), user: User = Depends(require_roles("HRAdmin"))):
    ...
    return dashboard_cache.get_or_set("dashboard:metrics", compute)
...
def charts(db: Session = Depends(get_db), user: User = Depends(require_roles("HRAdmin"))):
    ...
    return dashboard_cache.get_or_set("dashboard:charts", compute)
